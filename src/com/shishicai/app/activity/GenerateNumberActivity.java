...
public class GenerateNumberActivity extends Base2Activity implements OnClickListener{
	...
	private List<AwardReferInfo.ItemsBean.DataBeanX> mInfos;
	private ReferListAdapter adapter;
	private int num;
	private String[] singleDouble = {"单", "双"};
	private String[] bigSmall = {"大", "小"};
	private String[] longHu = {"龙", "虎"};
	private String[] name = {"第一球", "第二球", "第三球", "第四球", "第五球", "龙虎"};
	private Random r;
	private int[] lottery;
	...
	private List<AwardReferInfo.ItemsBean.DataBeanX.DataBean> data;
	private AwardReferInfo.ItemsBean.DataBeanX info;
	private AwardReferInfo.ItemsBean.DataBeanX.DataBean bean;
	...
	private void getLottery()
	{
		...
		for (int j = 0; j < 6; j++)
		{
			...
			info = new AwardReferInfo.ItemsBean.DataBeanX();
			info.setName(name[j]);
			data = new ArrayList<>();
			bean = new AwardReferInfo.ItemsBean.DataBeanX.DataBean();
			String code = Arrays.toString(lottery);
			code = code.substring(1, code.length() - 1);//r.nextInt(2) + ""
			bean.setState(0);
			if (j != 5)
			{
				bean.setResult(code);
			} else {
				bean.setResult(longHu[r.nextInt(2)]);
			}
			data.add(bean);
			bean = new AwardReferInfo.ItemsBean.DataBeanX.DataBean();
			bean.setResult(singleDouble[r.nextInt(2)]);
			bean.setState(0);
			data.add(bean);
			bean = new AwardReferInfo.ItemsBean.DataBeanX.DataBean();
			bean.setResult(bigSmall[r.nextInt(2)]);
			bean.setState(0);
			data.add(bean);
			info.setData(data);
			mInfos.add(info);
